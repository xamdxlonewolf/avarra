---
title: The Ages of the Turning
type: timeline
visibility: player
note_status: fleshed
status: active
tags: [history, timeline, eras, grafting, epic-6, moc]
aliases: [The Ages, How History Is Counted, The Era Spine, Cut-Years, The Grafting]
world: The Turning
reveals: [keystone-adjacent]
created: 2026-08-24
updated: 2026-08-30
---

# The Ages of the Turning

> **What this is.** How **The Turning** divides the past — the setting’s era spine. This is not a chronicle of kings and not a four-age cosmology dump. The load-bearing fact was already banked: once there was only the [[Turning Tree|Awakening Tree]], and a child's tenth-year Turning meant a *walk* to it; then grafts travelled, and her reach travelled with them. This note names the ages, how years are counted, and what that fact *did* to law, money, language, and the map. Lived texture of the road → [[The Walking Years]]. The hinge event and the spread → [[The First Cut]]. The present as leftover → [[The Years of Hands]] · [[Settlement Seeds]]. Dated years that were not wood → [[The Other Count]].


## The organizing idea: two clocks, not four stacked ages

People in this world do not live in "the Fourth Age." They keep **two overlapping clocks**, and which one they glance at tells you where they stand.

**Clock 1 — How you Turned** (personal, generational). Did you walk to [[Maiethorn|Thaeloren]] (or to a neighbour who had already walked), or did you stand under a town graft? Ordinary families date themselves this way: *we are a Hands-house; grandmother still talks of the road.* A [[Long-Lived]] who says *"I walked"* is dating the room.

**Clock 2 — Where the wood has reached** (geographic, unfinished). The **Grafting** is not a closed golden age. It is a *wave of living wood* that started at the origin, filled [[Maiethorn]], crossed to [[Strandoren]] by sea, and is **still arriving** on [[Heskoren]]. [[The Sundering Isle|Kumbaan]] is off the wave entirely. Present-day thin reach is not "history's scar." It is the front of a process that has not finished.

Both clocks sit on an unnamed preface: [[Before the Walk]]. The Tree was already there. No honest count starts it.

> **What this is not.** It is not a civilizing mission and not a golden age of pilgrimage that grafts then "improved." The Walking Years were a *logistics problem that killed children on the road.* The Grafting was *horticulture that people later sanctified.* The Seat now narrates both as her will travelling. Folk memory is less tidy. Keep the romance and the graves in the same sentence.

## The ages at a glance

| | Everyday | Liturgical 🔒 | Says | What it *is* |
|---|---|---|---|---|
| Preface | **[[Before the Walk]]** | *(mostly unnamed)* · [[The Watching]] keep **the Before** | — | The Tree already stood. Kinds, doors, and seeing without a mother's name. Uncounted on purpose. |
| Clock 1, old default | **[[The Walking Years]]** | **Brenvaeth** *(BREN-vaeth)* | "the hearth gone to waiting" | You walked to Thaeloren or you stayed [[Law and Citizenship\|Kept]] for want of the trip. Far lands ran heavy on the Kept and the [[The Premise\|Struck]]. |
| The hinge | **[[The First Cut]]** | **Eoloren** *(EOL-or-en)* | "the witnessed hinge" | Someone cut living wood and it *took*. Who, is argued. That it happened, is not. |
| Clock 2 | **the Grafting** | *(process, not an age you exit)* | — | The carrying of hands. Still unfinished at the [[Heskoren\|Sundered Reach]]. Never crossed the storm-wall. |
| Clock 1, new default | **[[The Years of Hands]]** | **Ornthael** *(ORN-thael)* | "Tree-land" | A town Tree is the ordinary way to Turn. Pilgrimage remains — as devotion, prestige, or necessity where a graft has failed. |

The liturgical pair is a deliberate inversion of the [[The Tree-Wardens|Road-hands']] own name. **[[The Old Tongue|Thaelvaeth]]** is "the Tree gone to the waiting" — wood carried *out*. **Brenvaeth** is "the hearth gone to the waiting" — people walking *in*. The Grafting is the moment the world stopped sending hearths to the Tree and started sending the Tree to hearths. Names 🔒 (user-approved 2026-08-24).

## How years are numbered (the name-reveals-stance lever, on history itself)

There is **no universal year-zero.** Same twelve months; three new-year's days ([[The Reckoning of the Year]]). Same past; **four honest ways to count it.** The number you write on a debt is a theology.

| Who | What they count from | What it sounds like |
|---|---|---|
| [[The Tree-Wardens\|First Seat]] / devout clerks | **the First Cut** — **Cut-years** (liturg. the *Eoloren-count*) | "the 387th Cut-year" |
| [[Polity Archetypes\|Threnmaieth]] | Cut-years *and* a **crown-count** from [[The Two Papers]] (C.Y. 67; present **Crown-year 320**) | a number that wants to be a roll |
| [[Polity Archetypes\|Lestrand]] / [[Economy and the Tithe\|deathless houses]] | a **house-founding**, a note-issue, a contract-term | "the 307th summer of the White Note" (present); the archival conversion is the **200th** (Ledan's query, C.Y. 280) |
| [[Polity Archetypes\|Vaethorn]] | **the year our graft took** — or the last *actual* Turning-Week | a child "turns the year" when the local Tree last managed a fall (already how they start the calendar) |
| [[The Watching]] | they **refuse the Cut-count** | the overlay is not an epoch; the Tree was already seeing |
| [[The Old Ways]] | a closed door, a drowning-year, a guest that took | the Hands are when *guests* arrived, not when history began |
| [[The Sundering Isle\|Kumbaan]] | **moons** | the mainland ages are a sailor's chart they do not owe |

**Present (🔒):** **Cut-year 387** — the 387th summer since the First Cut. Vault shorthand **C.Y. 387**. Not a round number on purpose.

**What you may not date.** The [[Turning Tree|Awakening Tree]] is **old beyond reliable dating.** [[Before the Walk]] has no start. The *nature* of the [[Is the Leaf-Mother Real|Mother's limit]] stays open. Who made the First Cut stays contested ([[The First Cut]]).

### Spread of the wave (🔒 shape and bands)

Enough to keep geography honest. Not a campaign almanac.

| Cut-years | Where the wood went |
|---|---|
| **C.Y. 0** | [[The First Cut]] — near Thaeloren, [[Maiethorn]] |
| **C.Y. 0–80** | Maiethorn filling. Origin-towns first; the Rain-Shadow slower. |
| **C.Y. 40–160** | [[Strandoren]] by sea — wealthy coasts first, deep interior later. |
| **C.Y. 200–387** | [[Heskoren]] — still incomplete. Luckiest towns have had a local Turning for a few generations; whole districts still wait. |
| **never** | [[The Sundering Isle\|Kumbaan]] |

A freshly planted scion still takes **a generation** to hold a Leaf-Fall ([[Turning Tree]]). So "a graft arrived" and "this town Turns under its own canopy" are twenty years apart. Heskoren's waiting is not only the queue. It is the queue *plus the childhood of the wood.*

## What the two clocks did (spine consequences)

Traced with `systemic-worldbuilding`. Lived texture belongs to later stories; these are the load-bearing knock-ons so law, money, and the map do not have to be retrofitted.

**Law.** [[Law and Citizenship|Citizenship-by-local-witness]] is a **Hands-era default.** In the Walking Years the town that *saw you Turn* was the crowd under Thaeloren — or no one, if you never arrived. Far children who stayed Kept were not "unmarked on a roll"; they were **unwitnessed as Given because they did not make the trip.** Vouching-at-a-distance (a [[Long-Lived]] who remembers the origin-fall, a [[Bound]] word, a [[Far-Voiced]] feeling) was invented to *carry the origin's witness home.* The unvouched Struck are older than the grafts: a becoming on a far threshold was always unwitnessed. The Grafting did not invent that unease. It *localized* who is expected to have been seen.

**Money.** Many [[Economy and the Tithe|deathless houses]] began as **road-houses** — inns, endowments, and century-credit for families who had to walk. When the walk stopped being required in the core, the houses still had Long-Lived memory of every debtor on the road, and paper that outlived the pilgrimage-season. They became banks. The endow-not-inherit loop is older than the Hands; the *trusted note* is a Walking-era instrument that outlived its road. This is a `world-fates` reformation already in the past: the same houses, a different fortune. Do not treat present banking as designed last week.

**Clergy.** [[The Tree-Wardens|Cutting-leave]] is how the First Seat **captured a practice that began without it.** The first successful cut was argued as theft, as mercy, as the Tree offering a branch, as old horticulture the Motherfaith later named. The Seat won the *authorization*. The [[The Tree-Wardens|scion-queue]] is the Grafting's child — the bottleneck that appears when someone decides *which town next.*

**Language.** The pilgrimage was a **conserving pull.** Once a year the far-flung walked to the same grove and heard the same liturgy, so [[The Old Tongue]] stayed nearer itself along the road. Local Trees let daughters grow *in place.* This is why drift tracks Tree-thinness *and* why [[Polity Archetypes|Lestrand]] eroded so far so fast: trade, a local canopy, and no need to walk to the Motherwood. The three daughter-drifts are Hands-era weather sitting on a Walking-era root. Deep grammar still deferred.

**Geography.** The [[The World Frame|reach-gradient]] *is* the wave, frozen at C.Y. 387. [[Maiethorn]] is old Hands. [[Strandoren]] is mature Hands with a commercial aftertaste. [[Heskoren]] is the Grafting *this decade.* [[The Sundering Isle|Kumbaan]] never entered the clocks. In-world this reads as distance-and-history (true as far as it goes). The deeper cause — [[Is the Leaf-Mother Real|her reach has an edge]] — stays GM-side.

**Faith.** [[The Watching]] remember [[Before the Walk]] as *the Before* — seeing without a mother's name — and will not start history at a Cut. [[The Old Ways]] were already becoming before any pilgrim road; the Hands are when *guests* arrived late. [[The Fair Hand]] dates contracts from a signing, not from a wound in wood; the Grafting is a supply-chain fact. [[The Open Table]] owes the ages nothing.

## Visibility markers (what a character notices)

- A [[Long-Lived]] who says *I walked*, and a room that goes quiet — Clock 1, spoken as biography.
- A **mile-shrine** on a road that now ends at a town Tree: a stone for a Turning that no longer has to happen there. The devout still leave a ribbon. The town children climb it.
- **Empty pilgrim-beds** in an old inn's upper room, let now to merchants. The deathless house that owns the inn still dates its notes from the founding-summer the beds were full.
- Three dates for one summer, written on one debt: a Cut-year, a house-year, a "year the graft took." A fourth if the note survived [[The Grey Summer]]. See the clerk's query below — it is **archival** (C.Y. 280), and Ledan is still the house's writing-mouth.
- A [[Heskoren]] hamlet whose ten-year-olds still walk to a neighbour. They are not doing history. They are doing *this year.*
- A [[Phoenix]] named as an era-marker — *the Phoenix of the last Walking summer* — without anyone agreeing which summer that was.

## In-world text

*A query from a White-Note clerk to a Speaker of the First Seat, **Cut-year 280** — the house's 200th summer. Filed in the founding-book; still the conversion the desk reaches for. Ledan is [[Long-Lived]]; Mataero, in C.Y. 387, is still junior to the same mouth. The verso has been folded around a second number. `oblique-worldbuilding`.*

> *Ledan of the White Note, to the Speaker who keeps the Eoloren-count.*
>
> *The house writes a century-loan from our founding-summer, which we call the 200th of the White Note. The debtor in Wetorn writes the same summer as the year their graft took. Your college will write it as the 280th Cut-year if I have the conversion right, and as the 279th if I have it wrong.*
>
> *Which number should stand on the note so that in fifty years the same person still owes us? We do not ask which summer was holy. We ask which summer a court will still be able to find.*

The clerk cannot afford to see that the three numbers are three theologies. The Speaker who answers (if they answer) will name a wood. The Vaethorn debtor will name a taking. The loan will still come due. In C.Y. 387 the house is in its **307th** summer, and the same mouth still asks which number a court can find.

## What this epic does not lock

- **Who made the First Cut** — contested on purpose. Folk / clergy / devout / Watching / Old Ways each have a lived story ([[The First Cut]]). The spine does not pick a face. Pick only if play needs a tomb.
- **When the Tree first stood** — uncounted. [[The Watching]] will fight any date.
- **The nature of her limit** — [[Is the Leaf-Mother Real|locked that it exists, open on what it is]]. That reach *grew with the grafts* is already keystone evidence of a hand. It is not an explanation of the wound, the finitude, or the rationing. Do not smuggle a cause in as history.
- **A creation myth, a first king, a war of the ages.** The Closed Seat was a gate that grew a country, not a first king ([[The Closing]]). Dated non-Tree weather lives in [[The Other Count]] — a war, a split of paper, a fever, a flood, a hush — and does not become a fourth age. The spine is still two clocks.

## Links
- **Eras & the hinge:** [[Before the Walk]] · [[The Walking Years]] · [[The Child Who Counted Stones]] · [[The First Cut]] · [[The Years of Hands]] · [[The Child Who Climbed the Stone]] · [[Settlement Seeds]]
- **The Other Count:** [[The Closing]] · [[The Two Papers]] · [[The Grey Summer]] · [[The Thaw-Break]] · [[The Hinge Hush]] · leftovers [[The Low Wall]] · [[The Seeing-Ring]] · [[The Dry Stair]]
- [[The Reckoning of the Year]] — months, solstice, three new-year's days; Cut-years live there too
- [[Turning Tree]] · [[The Tree-Wardens]] · [[The Leaf-Mother]] · [[Is the Leaf-Mother Real]]
- [[The World Frame]] · [[Maiethorn]] · [[Strandoren]] · [[Heskoren]] · [[The Sundering Isle]]
- [[Law and Citizenship]] · [[Economy and the Tithe]] · [[The Old Tongue]] · [[Naming in the Turning]]
- [[The Watching]] · [[The Old Ways]] · [[The Fair Hand]] · [[The Open Table]]
- [[Long-Lived]] · [[Phoenix]] · [[The Branch That Came Away]] · [[The White Note House]] · [[The Third Hearth]] · [[The Walled Book]] · [[02 - History]] · [[Roadmap]]

## GM Notes

### Production notes (P2.1)

Moved from the player-facing body during residual export polish:

- > **What this is.** How **The Turning** divides the past — the era spine for [[Roadmap|Epic 6]]. This is not a chronicle of kings and not a four-age cosmology dump. The load-bearing fact was already banked: once there was only the [[Turning Tree|Awakening Tree]], and a child's tenth-year Turning meant a *walk* to it; then grafts travelled, and her reach travelled with them. This note names the ages, how years are counted, and what that fact *did* to law, money, language, and the map. Lived texture of the road → [[The Walking Years]] (Story 6.2 ✅). The hinge event and the spread → [[The First Cut]] (Story 6.3 ✅). The present as leftover → [[The Years of Hands]] (Story 6.4 ✅) · [[Settlement Seeds]]. Dated years that were not wood → [[The Other Count]] (Story R.9).
- > **Canon status.** 🔒 **Locked 2026-08-24 (user-approved):** two clocks, unnamed preface, Grafting as a still-moving wave, no universal year-zero, dating reveals stance, names (*Brenvaeth · Eoloren · Ornthael*), present **C.Y. 387**, spread-shape (Maiethorn first, Strandoren by sea, Heskoren last and live, Kumbaan never). Still open: who made the First Cut; [[Is the Leaf-Mother Real|the nature of her limit]]. Built with `systemic-worldbuilding` and `world-fates`.
- **Present (🔒):** **Cut-year 387** — the 387th summer since the First Cut. Vault shorthand **C.Y. 387**. Not a round number on purpose. Story 6.3 dated the *carrying* inside the spread-bands; Story R.9 dated the *other weather* ([[The Other Count]]) without sliding the present.
- Enough to keep geography honest. Not a campaign almanac. Story 6.3 fleshed *how* inside these bands; it did not rewrite *when*.
- - **The Other Count (Story R.9):** [[The Closing]] · [[The Two Papers]] · [[The Grey Summer]] · [[The Thaw-Break]] · [[The Hinge Hush]] · leftovers [[The Low Wall]] · [[The Seeing-Ring]] · [[The Dry Stair]]
- - [[Long-Lived]] · [[Phoenix]] · [[The Branch That Came Away]] · [[The White Note House]] · [[The Third Hearth]] · [[The Walled Book]] · [[02 - History]] · [[Roadmap]] (Epic 6 complete; Story R.9)
*(Stripped on player-safe export. Keyed to the keystone via `reveals: [keystone-adjacent]`.)*

- **The two clocks protect the keystone without confirming it.** Clock 2 (the wave) is the player-facing face of "reach grew with the grafts" — the quiet argument that a *hand*, not a climate, is behind the thinning. Skeptics still win the mundane reading (young/sick/few grafts, distance, history). Both readings share the same map. Do not let a player-facing era note assert the mind.
- **The nature of the limit stays open.** Story 6.1 may show that carrying wood *moved the edge*. It may not say *why she needed the wood carried* (finite by nature, spent from a wound, rationing, holding something else up). Lock the ache. Do not lock the cause.
- **The Grafting is not a redemption arc.** Local Trees relocated who gets a local witness and who waits on a queue. The Seat's phrase *her will travelling* is a claim about the wood, not a verdict on the list.
- **Do not run the colonial version.** "We brought Trees to the benighted far" is the Seat's after-the-fact sermon and Lestrand's supply-chain story. The thing that happened is shorter and worse: children died on the road, someone cut a branch, it took, people copied it, the Seat claimed the copy-right. [[Heskoren]] still experiences the wave as *waiting*, not as having been given civilization. [[The Old Ways]] experience it as guests arriving late. Keep all three.
- **Historical fate-shifts (already happened — not proposals).** `world-fates` as philosophy, not a roll: (1) road-houses → deathless banks (reformation); (2) unsanctioned cutting → Cutting-leave and the queue (absorption of a heresy by an institution); (3) required pilgrimage → optional pilgrimage (the institution survives by changing function); (4) Closed Seat → Inner Close ([[The Closing]]); hymn-country → two papers; quay-fever → Trenledd roll / Brenledd compact; hinge-war → Hush-rate. Present-day fate-pressure is tabled in [[The Years of Hands]] (noted, not rolled). Do not collapse any without a later call.
- **Long-Lived memory is the era-marker the calendar refused to be.** At C.Y. 387, plenty of deathless people remember the late Walking as biography. They are walking archives, and they do not agree — memory is perfect; *meaning* is not. A Long-Lived who walked as a child and a Seat chronicle that starts at the Cut will fight about the same summer. That fight is play. Do not have "the" Long-Lived account.
- **Kumbaan is the control.** If a later story "finds an old graft on the Isle," it breaks the isolate and the Open Table. Road-hands do not bring the Mother across the storm-wall. Already locked in Epic 5; history must not undo it.
- **Present-year and spread-bands are locked (2026-08-24).** C.Y. 387; Maiethorn first, Strandoren by sea, Heskoren last and live, Kumbaan never. Story 6.3 dated the carrying *inside* those bands (generation of argument · sea-as-cargo · Long-Mile dead wood · paying next). Do not slide the present, and do not invert the order.
- **Hooks.** A debt with three dates (or four, if Grey + 1); a Long-Lived who walked and a town that wants that memory as citizenship; a mile-shrine a Hands-town wants to pull down; a Speaker asked to convert a house-year to a Cut-year for a court; a Heskoren cohort still walking while a neighbouring canopy is one summer from its first fall; Ledan's unanswered 280 query still on the desk in 387.
