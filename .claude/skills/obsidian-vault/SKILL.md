...
- Every note begins with **YAML front-matter**. Base keys on *every* note: `title`, `type`, `visibility` (`player|gm`), `note_status` (`stub|draft|fleshed|canon`), `tags`, `aliases`, `created`, `updated`. Common optional: `status` (in-world state), `world` (`avarra|avera|vara`), `reveals: []`. Plus kind-specific keys — copy the matching template from `00 - Core/Meta/Templates/`.
