---
title: Named Ground
type: region
subtype: gazetteer
visibility: player
note_status: fleshed
status: active
tags: [world, geography, gazetteer, epic-r, story-r10]
aliases: [Named Terrain, Rivers and the Rain-Wall, Travel Times]
world: The Turning
reveals: [keystone-adjacent]
created: 2026-08-30
updated: 2026-08-30
---

# Named Ground

> **What this is.** The physical names the settlements and the Other Count were already using as common nouns: the water between the Old World pair, the range that makes the dry east, four rivers a GM can point at, and a travel table built from the walks that already existed. Everyday first. No new liturgy. Seed `20260831` (LCG; lists shuffled; names picked from the **middle**, never the first thought). The labelled picture is [[The Known Map]]. How to *draw* a map lives in `14 - Assets/Maps/` — not here. Paintings are not a second gazetteer.

> **Canon status.** 🟡 **Named ground (Story R.10).** 🔒 **Does not add a sixteenth power.** 🔒 **Does not date the Tree.** 🔒 **[[The Sundering Isle|Kumbaan]] never took a graft** — dead wood is not a Tree. Deep climate/ecology still deferred. Adventure-site depth stays [[Roadmap|Story R.11]].

## How to say it at the table

Speak the common-tongue name. The liturgical reconstruction, if anyone bothers, is a later clerk's hobby. Stance still shows: a devout mouth keeps *th* in the land-names around the name; a dock mouth does not.

| Everyday | What it is | Who says something else |
|---|---|---|
| **the Old Crossing** | the busy water between [[Maiethorn]] and [[Strandoren]] | Lestrand: *the crossing.* Vaethorn: *the waiting-water.* Sailors: *the Crossing.* Threnmaieth wants it on a roll. |
| **the West Water** | the wider sea between Strandoren and [[Heskoren]] | already the Night Shore's word; the Hush's west clause lives here |
| **the Rain-Wall** | Maiethorn's central range | [[Lirorn]]-folk: *the Thaw-Wall.* Clerks: *the Divide.* |
| **the Noon Pass** | the old high road over the Rain-Wall (lost its stone in C.Y. 233) | the courier's *noon-pass*; the water-line is still on the stone |
| **the Shelf-gate** | the pass Narol took after the Break | Threnmaieth named *the new one*; Lirorn charges **Break-rate** |
| **the Core-thaw** | the river [[Maiethlir]] sits on | clerks: *snowmelt.* Watchers will not. |
| **the Well-wash** | Rain-Shadow seasonal water | dry most years; silt in a bad thaw |
| **the Rise-water** | the stream from [[Harrow's Green]] to the hamlets' ford | the Seat still says *the ford* |
| **the Chart-run** | Strandoren's interior river to [[Orentel]]'s estuary | Lestrand files it as inland tickets reaching salt |

Seed `20260831`. Mid-picks: *the Old Crossing · the Rain-Wall / the Thaw-Wall · the Core-thaw · the Rise-water · the Chart-run · the Well-wash · the Shelf-gate.* **the Noon Pass** and **the West Water** were already in the mouth; this pass locked them.

## The Old Crossing

The oldest trade route in the world. Narrow enough that a fair-weather hull does it in a handful of days; busy enough that a late storm in Hale-month is still a Kept child. [[Eolstrand]] is the Motherland face. [[Orentel]] is the Shore-lands face. The Salt Walk was this water, then the Near Mile.

It is not "inner" because a cartographer said so. It is **old** and **close**. The two continents are the Old World pair because this water made them one argument.

What happened on it, dated: [[The Grey Summer]] (C.Y. 171) tied the quays. [[The Hinge Hush]] (C.Y. 304) is the peace still charged as the **Hush-rate**.

## The West Water

Wider than the Crossing. This is the Long Mile's sea-leg, the graft-pot's worse berth, [[Netstrand]]'s leftover. Heskoren's frontier coast faces it back toward Strandoren. Beyond the last capes, the storm-wall. The Hush's west-water clause lives here: Netstrand quotes the far; Saelvaeth may take a pot without a waiting-due; Vaethorn may bless and may not levy; Ornled is a beach-fee.

Do not book [[The Sundering Isle|Kumbaan]] on this water. A wrecked pot in a sailor's story is allowed. A taking is not.

## The Rain-Wall (Lirorn: the Thaw-Wall)

A highland spine down [[Maiethorn]]'s middle. West-face weather is wet and four-seasoned; the leeward **east** is the Rain-Shadow. The range is why [[Saelthael]] sows at wells. The range is why [[Lirorn]] can levy a thaw.

Fox-of-the-Snows and [[Tengu]] hearths sit densest on the high snow. That is terrain, not a flag ([[Kinds of the Turning]]).

Two passes a table needs:

- **the Noon Pass** — the old high road. [[The Thaw-Break]] put a water-line over the ribbon-notch. Children rub it. The road to it was gone for two summers.
- **the Shelf-gate** — the road that was left. Narol of the Pass took the toll when the old stone sat under the water-line. Threnmaieth wanted the new one counted. Lirorn charged them for the water anyway.

Heskoren's highland spines are a *different* weather. Do not hang the Rain-Wall on the frontier.

## Four rivers

### the Core-thaw

Snowmelt off the Rain-Wall's west face. Runs toward the Old Crossing. [[Maiethlir]] sat down where the water slowed enough to hold a Hand — a day's living-earth from the Motherwood, on the *river road*, not [[Orenbren]]'s Near Mile.

The extra mile used to come through with this thaw. Clerks file the spring rise as snowmelt. **Inscrutable, on purpose (already on the seat):** the Core-thaw runs louder the week before Leaf-Fall. Watchers do not file it. Rithim Speaks over it.

In a high Liren the lodging-road meets a near branch of the same water. Thilim's slate — *the summer the ford ran high* — is this river in a bad year, not a fifth name.

[[The Thaw-Break]] (C.Y. 233) is what happens when the Rain-Wall does not thaw as it usually thaws. The Core-thaw took a month in seven days.

### the Well-wash

East of the Rain-Wall: a seasonal run that is a river in a kind Liren and a silt-line in a cruel one. Towns cluster at wells because this water cannot be a civic certainty. [[Ornsael]] drinks a well, not this wash; the wash is why the well matters. The well-town the Break silted is of this weather. [[The Dry Stair]] climbs above a different well-town and is not a river-shrine.

### the Rise-water

A low stream off Harrow's rise. Downstream it is the ford the Seat used as a geography because the Seat will not learn Brenod, Vaelun, Ornath. In clear weather the hamlets can see the canopy from the water. No stone on the waiting side. The cup still goes on the ford-rock.

### the Chart-run

[[Strandoren]]'s interior river-plain, running to [[Orentel]]'s estuary. The First Cut's "river-days" are this water: coasts took by sea; inland waited on a cart *and* this run. [[Trenledd]] sits on the wealthy stretch that got filed. Food on the Salt Quay is this plain, the Crossing's catch, and whatever the hinge pays.

## Climate bands (compact)

Deep ecology is still deferred. This is enough to know what a week feels like.

| Band | Where | Weather |
|---|---|---|
| Crossing-shore | both faces of the Old Crossing | mild, wet-temperate; Hale-month storms decide a Salt Walk |
| Mother-core | Maiethorn west of the Rain-Wall | four true seasons; cathedral-country; winters that make High Solstice mean something |
| Rain-Wall / Thaw-Wall | [[Lirorn]] | snow, shelves, a thaw that is a civic year |
| Rain-Shadow | [[Saelthael]] east | dry hills, wells, the Well-wash or nothing |
| Shore-lands | Strandoren coasts and the Chart-run | maritime, indented, the sea as highway |
| Night Shore | [[Netstrand]] | open-ocean; west-weather; watches for hulls that do not arrive |
| Sundered upland | Heskoren | cooler, wilder, harder winters; civilization is a map of surviving Trees |
| Storm-wall / Kumbaan | [[The Sundering Isle]] | mild wet under cloud; bright nights; the wall is the climate |

## Travel times

The unit is a **day's walk** — the interval a mile-shrine already marks ([[The Walking Years]]). A horse does not invent a different world; it buys you the same road with fewer blisters. A ship buys you the water the walks already named.

The three walks, converted:

| Walk | What it was | Fair-weather band |
|---|---|---|
| **Near Mile** | Maiethorn inland to Thaeloren | **3–12 days** (a couple of weeks at the long end) |
| **Salt Walk** | Old Crossing, then the Near Mile | **3–5 days' sail** + the inland days → typically **1–3 weeks** |
| **Long Mile** | West Water, then Heskoren inland | **weeks to months** — figure **6–12 weeks** for a coast-to-grove attempt |

[[The Sundering Isle|Kumbaan]] is not on this table. No walk crossed the storm-wall.

### Places, relative to the walks

| From | To | How | Fair weather |
|---|---|---|---|
| [[Turning Tree\|Thaeloren]] | [[The Walled Book\|Inner Close]] | walk | **1 day** |
| Thaeloren | [[Maiethlir]] | river road along the Core-thaw | **1–2 days** |
| Thaeloren | [[The Third Hearth]] | Near Mile | **3 days** |
| Third Hearth | Brenthael | Near Mile | **1 day** further out |
| Thaeloren | [[The Low Wall]] | off the present Mile | **1–2 days** (not inside the Close) |
| Thaeloren | [[Eolstrand]] quays | Core-thaw and the west-road | **8–12 days** |
| Thaeloren | [[Ornsael]] | west-road over the Shelf-gate (or the Noon Pass, if you like a water-line) | **10–14 days** |
| Eolstrand quay | [[Orentel]] | Old Crossing | **3–5 days' sail** |
| Orentel | [[Trenledd]] interior | Chart-run | **4–8 days** |
| Orentel | [[Netstrand]] west quays | coasting | **8–14 days** |
| Netstrand | Heskoren frontier coast | West Water | **2–4 weeks** |
| Heskoren coast | [[Eolvaeth]] | inland vale | **1–2 weeks** |
| Eolvaeth | [[Harrow's Green]] | live-front roads | **2–3 weeks** (Eolvaeth cannot see Harrow's, and does not want to) |
| Harrow's Green | [[The Three Hamlets Past the Ford]] | the Rise-water | **hours to 1 day** (the canopy is a dark on the far slope) |
| Any mainland quay | Kumbaan | storm-wall | **not a timetable** |

A Hale-month storm on the Old Crossing can turn a Salt Walk into a missed year. A thaw-week on the Rain-Wall can turn the Shelf-gate into a lake ([[The Thaw-Break]] closed roads **two summers**). The table is weather, not a promise.

## The remnant stays where the sitting was

The surviving Kept regime is **not** a sixteenth flag and **not** a replacement of [[Eolstrand]].

The Closed Seat was an origin-gate on the grove ([[The Closing]]). Hildal's Retreat closed walls **one day's walk from the Motherwood**, inside [[Orenbren]] lodging-country. That is the [[The Walled Book|Inner Close]]. A shore-flag cannot carry a remnant that sat the *grove*. Eolstrand is the hinge of a different war ([[The Hinge Hush]]). Leave both jobs where they are.

## Links
- [[The Known Map]] — the labelled picture · [[The World Frame]] — the reach-gradient
- [[Maiethorn]] · [[Strandoren]] · [[Heskoren]] · [[The Sundering Isle]]
- [[Powers of the Turning]] · [[The Walking Years]] · [[The Other Count]]
- [[The Thaw-Break]] · [[The Hinge Hush]] · [[The Grey Summer]] · [[The Closing]]
- [[Maiethlir]] · [[Orentel]] · [[Eolvaeth]] · [[Harrow's Green]] · [[The Three Hamlets Past the Ford]] · [[The Third Hearth]] · [[Ornsael]]
- [[The Walled Book]] · [[Orenbren]] · [[Eolstrand]] · [[Lirorn]] · [[Netstrand]]
- [[01 - World]] · [[Roadmap]] (Story R.10)

## GM Notes
*(Stripped on player-safe export. Keyed to the keystone via `reveals: [keystone-adjacent]`.)*

- **Names are handles, not a new cosmology.** The reach-gradient is still the load-bearing map. Distances make the walks expensive; they do not confirm a mind.
- **Kumbaan never.** Align every later sentence with [[The First Cut]]: wrecked pots allowed; a taking is not.
- **Inner Close stays in Orenbren.** The geography pass is closed. Do not promote. Do not move the Book to the Hinge-Shore to tidy a stub.
- **The Core-thaw's loud week is already Maiethlir's inscrutable.** Do not explain it here.
- **Hooks.** A Hush-rate on a necessity-family; a Shelf-gate closed in Liren; a Rise-water cup the Seat will not name; a Chart-run delay that makes a Grey-verso due; a Well-wash year that looks like Clock 2 at the origin.
