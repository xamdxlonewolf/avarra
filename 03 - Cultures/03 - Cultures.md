---
title: 03 - Cultures
type: moc
visibility: player
note_status: stub
tags: [moc]
aliases: []
created: 2026-08-11
updated: 2026-08-30
---

# 03 - Cultures

Peoples, languages, customs, traditions, social structures, and naming conventions.

## Contents
<!-- Link notes in this section here as they are created. Aggregate; do not duplicate. -->

### Custom Kinds (born-ancestries unique to this world)
- [[Kinds of the Turning]] — how Kinds sit on the map: **hearths, not nations**; the other kitchen; mixed-Kind children
- [[Kind Heritage]] — Hearth-Mark, Mixed Ancestry, one surprise keyword
- [[Kitsune]] — fox-born; three hearths (Wilds / Snows / Sands); locked, 3 features
- [[Selkie]] — sea-born; coastal hearth; locked, 3 features
- [[Tengu]] — mountain-born crow-folk; ridge hearth; locked, 3 features
- [[Yumboe]] — the good people of [[The Sundering Isle|Kumbaan]]; mythic-rare off-isle; locked, 3 features
- *(These sit alongside Daggerheart's 18 stock ancestries — see [[Build Plan]]. Stock PCs take a Hearth-Mark.)*

### Social Structures
- [[Law and Citizenship]] — the universal legal grammar of a world where 3 of 4 carry a Condition. Citizenship is *witnessed, not recorded* (a hearth saw you Turn; proof runs through vouching people, not papers); city witnessing by hearth-stand; road-word and the watch's three questions; crime ladder under the Inviolate Will; guild safety-tickets for hazardous *uses* (ticketable pool ≠ census). Polity variation on three independent axes (theology / reach / governance).
- [[Economy and the Tithe]] — the universal economic grammar. The Tithe organizes vocation and a civic utility (wells, not granaries); self-paying vs provided-for Tithes (a spectrum, with Far-Voiced & Returned on the seam); why Taken-In live in stone cities; labor-by-Condition; the Kept as free generalist labor; deathless banking houses & the endow-not-inherit loop; center/periphery; shadow economy. Lived official faces → [[Tithe-Infrastructure]]. Lived illicit faces → [[The Slide]] · [[The Holding Desk]] · [[The Standing Trade]].
- [[Daily Life]] — the universal grammar of everyday life. Marriage & family as a spectrum (mundane-mixed → the aching Long-Lived marriage you outlive); medicine (Stilled surgeons, Returned tending the dead — miraculous, unequally distributed); city design as principles (the Tree at the centre, city Turning by hearth, Tithe-provision as civic utility, the watch on the street). Where the ~5%-scary dial sits at eye level.
- [[Polity Archetypes]] — three worked polities at *different corners* of the theology/reach/governance axes, proving the universal grammar above flexes: **Vaethorn** (the Waiting Lands — warm/poor/faithful pilgrim edge), **Lestrand** (the Ledger Coast — rich/cool secular merchant power), **Threnmaieth** (the Tallied Crown — sanctified surveillance, the darkest corner). Each pair shares exactly one axis, so the set proves the three are independent. The other twelve named-stubs → [[Powers of the Turning]].

### Languages & Naming
- [[The Old Tongue]] — *Maiethren*, the old liturgical root tongue of the [[Turning Tree|Awakening Tree]] (this world's "Latin"): warm/weighty phonology, pronunciation key, sacred lexicon, and **one root → three daughter drifts** (conserved core / worn frontier / eroded coast) — so *how a polity sounds reveals its stance*. Names the fifteen powers.
- [[Naming in the Turning]] — the naming-conventions companion: **the name a polity gives a shared institution reveals its stance** (venting-hall → *gift-hall* in devout Vaethorn / *release-house* on secular Lestrand / *counted hall* in surveillance-state Threnmaieth). The working dictionary of institution names + how to coin more.
- [[Naming People in the Turning]] — person-names: named by *place*; custom Kinds keep a hearth-register (Kusawe / Sakoa / Gonan / Yumboe). Seeded lists, collision rules.

### Conditions (acquired becomings — the "monster" layer)
- *(All 10 designed & locked with full mechanics — see [[Conditions]] in `09 - Creatures`.)*

## Related
- [[The Premise]] — the two-layer (Kind + Condition) model

## GM Notes

### Production notes (P2.1)

Moved from the player-facing body during residual export polish:

- - [[Kinds of the Turning]] — how Kinds sit on the map: **hearths, not nations** (Story 4.2); the other kitchen; mixed-Kind children
- - [[Kind Heritage]] — Hearth-Mark, Mixed Ancestry, one surprise keyword (Story R.3)
- - [[Law and Citizenship]] — the universal legal grammar of a world where 3 of 4 carry a Condition. Citizenship is *witnessed, not recorded* (a hearth saw you Turn; proof runs through vouching people, not papers); city witnessing by hearth-stand; road-word and the watch's three questions; crime ladder under the Inviolate Will; guild safety-tickets for hazardous *uses* (ticketable pool ≠ census). Polity variation on three independent axes (theology / reach / governance). *(Epic 2, Story 2.1; Story R.6.)*
- - [[Economy and the Tithe]] — the universal economic grammar. The Tithe organizes vocation and a civic utility (wells, not granaries); self-paying vs provided-for Tithes (a spectrum, with Far-Voiced & Returned on the seam); why Taken-In live in stone cities; labor-by-Condition; the Kept as free generalist labor; deathless banking houses & the endow-not-inherit loop; center/periphery; shadow economy. Lived official faces → [[Tithe-Infrastructure]]. Lived illicit faces → [[The Slide]] · [[The Holding Desk]] · [[The Standing Trade]]. *(Epic 2, Story 2.2; lived houses Epic 5 / Story R.7; Story R.6.)*
- - [[Daily Life]] — the universal grammar of everyday life. Marriage & family as a spectrum (mundane-mixed → the aching Long-Lived marriage you outlive); medicine (Stilled surgeons, Returned tending the dead — miraculous, unequally distributed); city design as principles (the Tree at the centre, city Turning by hearth, Tithe-provision as civic utility, the watch on the street). Where the ~5%-scary dial sits at eye level. *(Epic 2, Story 2.3; Story R.6.)*
- - [[Polity Archetypes]] — three worked polities at *different corners* of the theology/reach/governance axes, proving the universal grammar above flexes: **Vaethorn** (the Waiting Lands — warm/poor/faithful pilgrim edge), **Lestrand** (the Ledger Coast — rich/cool secular merchant power), **Threnmaieth** (the Tallied Crown — sanctified surveillance, the darkest corner). Each pair shares exactly one axis, so the set proves the three are independent. *(Epic 2, Story 2.4.)* The other twelve named-stubs → [[Powers of the Turning]] (Story 7.1).
- - [[The Old Tongue]] — *Maiethren*, the old liturgical root tongue of the [[Turning Tree|Awakening Tree]] (this world's "Latin"): warm/weighty phonology, pronunciation key, sacred lexicon, and **one root → three daughter drifts** (conserved core / worn frontier / eroded coast) — so *how a polity sounds reveals its stance*. Names the fifteen powers. *(Epic 2 tail / Epic 4 / Story 7.1.)*
- - [[Naming in the Turning]] — the naming-conventions companion: **the name a polity gives a shared institution reveals its stance** (venting-hall → *gift-hall* in devout Vaethorn / *release-house* on secular Lestrand / *counted hall* in surveillance-state Threnmaieth). The working dictionary of institution names + how to coin more. *(Epic 2 tail / Epic 4.)*
- - [[Naming People in the Turning]] — person-names: named by *place*; custom Kinds keep a hearth-register (Kusawe / Sakoa / Gonan / Yumboe). Seeded lists, collision rules. *(Story 4.2.)*
