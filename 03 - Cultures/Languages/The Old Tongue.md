...
**Feel:** warm and weighty — a hall-and-hearth tongue with a thread of solemn, archaic gravity. Liquids and nasals carry the warmth; thorn (*th*) and long vowels carry the weight; no harsh consonant clusters, so it stays sayable at the table.

**Consonants:** m n · l r w j · p t k · b d g · f v s h · **þ (*th*, as in _thin_)**. No harsh clusters; *th* is the signature sacred sound (it is the first consonant to erode away in the secular daughters — see below).
...
- `ae` = **"ay"** (as in _day_) · `ei` = **"eye"** · `eo` = an **"ay-oh"** glide · `a` = _father_ · `e` = _bed_ · `i` = _machine_ · `o` = _go_ · `u` = _moon_.
